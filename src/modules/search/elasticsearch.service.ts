...
export class ElasticsearchService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.warn(
        `Failed to initialize Elasticsearch index: ${errorMessage}. The service will continue but search functionality may be limited.`,
      );
    }
  }
  ...
  private async ensureEmailIndex() {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Error ensuring Elasticsearch index: ${errorMessage}`,
      );
      throw error;
    }
  }
  ...
  async indexEmail(doc: EmailSearchDocument) {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Failed to index email ${doc.id}: ${errorMessage}`,
      );
      throw error;
    }
  }
  ...
  async bulkIndexEmails(docs: EmailSearchDocument[]) {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Failed to bulk index emails: ${errorMessage}`,
      );
      throw error;
    }
  }
